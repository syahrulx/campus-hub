...
public class MessageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String productId = request.getParameter("about");
        ...
        if (conversationWith != null && !conversationWith.isEmpty()) {
            ...
            if (productId != null && !productId.isEmpty()) {
                Product chatProduct = getProduct(productId);   // call your method
                request.setAttribute("chatProduct", chatProduct);
            }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        response.sendRedirect("messages?with=" + receiverId + "&about=" + productId); 
    }

    private List<Map<String, Object>> getConversations(String userId) {
        List<Map<String, Object>> conversations = new ArrayList<>();

        // One conversation per (other_user_id, product_id)
        String sql =
            "SELECT DISTINCT " +
            "  CASE WHEN m.sender_id = ? THEN m.receiver_id ELSE m.sender_id END AS other_user_id, " +
            "  u.name AS other_user_name, " +
            "  m.product_id AS product_id, " +
            "  (SELECT m2.content FROM APP.MESSAGE m2 WHERE " +
            ...
            "     ORDER BY m2.sent_at DESC FETCH FIRST 1 ROW ONLY " +
            "  ) AS last_message, " +
            "  (SELECT COUNT(*) FROM APP.MESSAGE m3 WHERE " +
            "     m3.sender_id = CASE WHEN m.sender_id = ? THEN m.receiver_id ELSE m.sender_id END " +
            "     AND m3.receiver_id = ? " +
            "     AND m3.product_id = m.product_id " +
        ...
        try (Connection conn = DatabaseHelper.getConnection();
            ...
            // All placeholders are the current userId (10 total)
        ...
        try (Connection conn = DatabaseHelper.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    msg.put("productId", rs.getString("product_id"));
