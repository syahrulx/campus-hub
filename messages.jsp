						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="convo" items="${conversations}">
									<a href="messages?with=${convo['userId']}&about=${convo['productId']}" class="block transition-all ${convo.userId == chatWithId ? 'bg-red-50/50 relative' : 'hover:bg-gray-50'}">
										<%-- TODO fix --%>
                                                                                <c:if test="${convo['userId'] == chatWithId && convo['productId'] == chatProductId}">
							...
							<div class="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white z-10">
								<div class="flex items-center gap-4">
									...
									<div>
										...
										<p class="text-[10px] text-gray-500 font-black uppercase tracking-widest flex items-center gap-1">
