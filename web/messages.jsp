				...
				<div class="flex-grow flex flex-col bg-white">
					<c:choose>
						...
						<c:otherwise>
							...
							<div class="relative" id="chatMenuWrapper">
								<button onclick="toggleChatMenu()" class="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 hover:text-primary hover:bg-red-50 transition-all">
									<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"></path></svg>
								</button>
								<div id="chatMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
									<form action="messages" method="POST" onsubmit="return confirm('Delete this chat? This cannot be undone.');">
										<input type="hidden" name="action" value="delete">
										<input type="hidden" name="otherUserId" value="${chatWithId}">
										<input type="hidden" name="productId" value="${currentProductId}">
										<button type="submit" class="w-full px-4 py-3 text-left text-sm font-bold text-red-500 hover:bg-red-50 flex items-center gap-3 transition-all">
					...
					<script>
						...
						document.addEventListener('click', function(e) {
							var wrapper = document.getElementById('chatMenuWrapper');
